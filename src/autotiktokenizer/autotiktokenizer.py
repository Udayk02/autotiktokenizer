...
class AutoTikTokenizer:
    ...
    def _download_from_hf_hub(
        ...
        assert (
            "config.json" in files_downloaded
        ), "config.json not found in downloaded files"
        assert (
            "tokenizer.json" in files_downloaded or "vocab.json" in files_downloaded
        ), "tokenizer.json not found in downloaded files"
        assert (
            "tokenizer_config.json" in files_downloaded
    ...
    def _get_mergeable_ranks(
        self, vocab: Dict[str, int], special_tokens: Dict[str, int], tokenizer_type: str
    ) -> Dict[str, int]:
    ...
    def _get_special_tokens(self, tokenizer: Dict) -> Dict[str, int]:
        ...
        try:
            special_tokens = {
                at["content"]: at["id"] for at in tokenizer["added_tokens"]
            }
        except KeyError:
    ...
    def _detect_tokenizer_type(self, tokenizer: Dict, tokenizer_config: Dict) -> str:
        ...
        """
        if tokenizer_config.get("tokenizer_class", "").lower() in [
            "berttokenizer",
            "wordpiece",
        ]:
            return "wordpiece"
    ...
    def from_pretrained(
        ...
        if os.path.isfile(os.path.join(tokenizer_name_or_path, "tokenizer.json")):
            tokenizer = instance._read_json(
                os.path.join(tokenizer_name_or_path, "tokenizer.json")
            )
        # Load from the HuggingFace Hub
        else:
            try:
                path = instance._download_from_hf_hub(
                    tokenizer_name_or_path, token, local_dir, force_download
                )
                tokenizer = instance._read_json(os.path.join(path, "tokenizer.json"))
            except Exception as e:
                print(
                    "Tokenizer could not be loaded from a local directory nor from the hub"
                )
                raise e
        ...
        tokenizer_config = instance._read_json(
        ...
        mergeable_ranks = instance._get_mergeable_ranks(
            vocab, special_tokens, tokenizer_type
        )

        # Get the pattern string
        pattern = instance._get_pattern_str(tokenizer_type)

        return instance._get_tiktoken_encoding(
            name, pattern, mergeable_ranks, special_tokens
        )
